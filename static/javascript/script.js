  ...
  if (question == 0) {
    ...
    $.get("/get", { msg: query }).done(function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        botHtml += "<a href='https://www.imdb.com/title/" + data[i][1] + "' target='_blank'><div class='movie-box'><img src='http://img.omdbapi.com/?i=" + data[i][1] + "&apikey=eae28505" + "'/><p class='movie-title'>" + data[i][0] + "</p><p class='movie-details'>" + formatRuntime(data[i][2]) + " • " + formatReleaseDate(data[i][3]) + " • IMBD: " + data[i][4] + "</p></div>";
      };
      botHtml += "</div>"
      $("#chatbox").append(botHtml);
