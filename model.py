...
credits_ = pd.read_csv("credits.csv",low_memory=False)
...
metadata['score'] = metadata.apply(calculate_score, axis=1)
...
features = ['cast', 'keywords', 'genres', 'overview']
for feature in features:
...
def make_recommendation(query, metadata=metadata):
  ...
  tfidf_matrix = tfidf_vectorizer.fit_transform(metadata['soup'])

  cosine_sim2 = cosine_similarity(tfidf_matrix[-1], tfidf_matrix[:-1])
  
  sim_scores = list(enumerate(cosine_sim2[-1,:]))
  sim_scores = sorted(sim_scores, key=lambda x: (x[1], metadata['score'].iloc[x[0]]), reverse=True)
